                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select name="parent" class="form-control">
                                                        ...
                                                        @foreach ($categories as $cat)
                                                            <option value="{{ $cat->id }}"
                                                                @selected($category->parent == $cat->id)>{{ $cat->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            @endif
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="input-group">
                                                    <div class="d-inline-block custom-control custom-radio mr-1">
                                                        <input type="radio" value="1" @checked($category->status == 'Active' || $category->status == 'مفعل')
                                                            name="status" class="custom-control-input" id="yes1">
                                                        <label class="custom-control-label"
                                                            for="yes1">{{ __('dashboard.active') }}</label>
                                                    </div>
                                                    <div class="d-inline-block custom-control custom-radio">
                                                        <input type="radio" value="0" @checked($category->status == 'Inactive' || $category->status == 'غير مفعل')
